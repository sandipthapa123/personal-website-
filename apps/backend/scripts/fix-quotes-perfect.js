...
// In TS template string:
// We use \\' to output \' into emitted JS
// When evaluated in browser JS, \' inside '...' evaluates to a single quote inside the string!

const cleanBrowserScript = `<script>
    'use strict';
...
// Test VM execution:
// When NestJS renders this TS string template:
// \\\' becomes \' in the HTML string sent to browser
const testEvaluated = cleanBrowserScript
  .replace('<script>', '')
  .replace('</script>', '')
...
try {
  ...
  const finalContent = content.substring(0, scriptStart) + cleanBrowserScript + content.substring(scriptEnd + scriptEndStr.length - 17);
