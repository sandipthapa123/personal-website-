...
<body>
  ...
  <main role="main" id="main-content">
    ...
    <div id="tab-settings" class="tab-section ${activeTab === 'settings' ? 'active' : ''}" role="tabpanel">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
        <div class="card">
          <div style="font-size:14px;font-weight:800;color:#fff;margin-bottom:14px;">🌐 Platform Identity</div>
          <div class="form-group"><label>Site Title</label><input type="text" id="setSiteTitle" value="Sandip Thapa Academic Platform" /></div>
          <div class="form-group"><label>Site Description</label><textarea id="setSiteDesc" rows="3">Legal Scholar, Human Rights Advocate &amp; Disability Accessibility Specialist</textarea></div>
          <div class="form-group"><label>Production Domain</label><input type="text" id="setDomain" value="thapasandip.com.np" /></div>
          <div class="form-group"><label>Default Locale</label><select id="setLocale"><option value="en">English (en)</option><option value="ne">Nepali (ne)</option></select></div>
          <button class="btn btn-primary" onclick="saveSettings()">Save Settings</button>
        </div>
        <div class="card">
          <div style="font-size:14px;font-weight:800;color:#fff;margin-bottom:14px;">🔐 Security</div>
          <div class="form-group"><label>New Admin Password</label><input type="password" id="setNewPass" placeholder="Leave blank to keep current" /></div>
  ...
  <script>
    ...
    function confirmDeleteMedia(id,name){showConfirmDlg({title:'Delete Media',message:'Delete "'+name+'"? Content links will break.',icon:'🗑️',destructive:true,okLabel:'Delete',onConfirm:function(){showMsg('Media deletion not yet wired to backend','info');}});}
    function quickUploadMedia(input){showMsg('Upload: POST multipart to /api/v1/media/upload','info');}
    ...
    function renderMenuCanvas(m){var c=document.getElementById('menuCanvas');if(!c)return;var items=menuData[m]||[];if(!items.length){c.innerHTML='<div class="empty-state"><div class="empty-icon">🗺️</div><div class="empty-text">No items</div></div>';return;}c.innerHTML=items.map(function(it,i){return'<div class="menu-item"><span style="color:#64748b;font-size:14px;">⠿</span><div style="flex:1;"><div class="menu-item-label">'+esc(it.label||it.title||'Item')+'</div><div class="menu-item-url">'+esc(it.url||it.targetUrl||'/')+'</div></div><button class="btn btn-secondary btn-xs" onclick="removeMenuItem('+i+')">×</button></div>';}).join('');}
