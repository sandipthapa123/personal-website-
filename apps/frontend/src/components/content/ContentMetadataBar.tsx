    ...
    <div className="bg-ink-elevated border border-ink-border rounded-xl p-6 my-6 text-sm text-ink-400 space-y-4 shadow-sm shadow-black/10">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 pb-4 border-b border-ink-border">
        ...
        <div>
          ...
          <span className="font-medium text-ink-100 inline-flex items-center gap-3">
            <span className="inline-flex items-center gap-1"><EyeIcon className="text-xs text-gold-text" /> {viewCount.toLocaleString('en-US')}</span>
            <span className="inline-flex items-center gap-1"><UsersIcon className="text-xs text-gold-text" /> {uniqueVisitorsCount.toLocaleString('en-US')}</span>
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
        <div>
          ...
          <span className="font-semibold text-gold-text">{dualDates.bsFormatted}</span>
        </div>
        <div>
      ...
      {tags && tags.length > 0 && (
        <div className="pt-2 flex flex-wrap items-center gap-2">
          ...
          {tags.map((tag) => (
            <span key={tag} className="px-2.5 py-0.5 text-xs font-medium bg-gold/10 text-gold-text rounded-full">
              {tag}
            </span>
          ))}
        </div>
